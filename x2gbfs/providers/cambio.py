...
class CambioProvider(BaseProvider):
    ...
    def _all_stations(self) -> list[dict[str, Any]]:
        response = get(self.STATIONS_URL.format(mandator_id=self.mandator_id, category=self.category))
        response.raise_for_status()
        ret: list[dict[str, Any]] = response.json()
        return ret

    def _all_vehicle_types(self) -> list[dict[str, Any]]:
        response = get(self.VEHICLE_TYPES_URL.format(mandator_id=self.mandator_id, category=self.category))
        response.raise_for_status()
        ret: list[dict[str, Any]] = response.json()
        return ret
